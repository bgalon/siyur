...
- [ ] T007 Apply the ADR-0025 card amendments to `docs/data/bundle-manifest.md`, `itinerary.md`, `route-leg.md`, `tile-source.md`, `poi-site.md`, **`area.md`** and `DATA-LICENSES.md` — the frozen-itinerary manifest slot, per-artifact hashes incl. `attribution.sha256`, the `manifest_sha256` canonicalization rule, `withheld[]`, `ItineraryV1.date`, `area.timezone`/`area.country_code`, `stop_order`/`leg_id` timeline addressing, `Story.observed_at`, the `opening_hours.js` → `opening-hours-py` swap.
...
- [ ] T007b **Reconcile the feasibility column shape before writing the migration.** `data-model.md` §6 specifies a single `feasibility jsonb` column with `CHECK (status <> 'approved' OR feasibility->>'ok' = 'true')`; ADR-0025 ruling 3 specifies `user_plan.feasible` + `user_plan.violations`; ADR-0023 defines the `status` enum. All three must agree **before** T009 writes `0005_user_plan` — a migration is the worst place to discover a disagreement.
...
- [ ] T008 Add `timezone` (IANA) + `country_code` to `area` in `commons/db.py`, derived **deterministically from the polygon at resolve time** in `planner/nodes/resolve_area.py` — never guessed, never model-supplied. This is the calendar/locale frame every planned time and every PH lookup depends on (ADR-0025 ruling 2).
  - **The derivation rule, pinned** (it was under-specified, and SC-009 genericity would have found it): resolve from the polygon's **`representative_point()`**, not its centroid — a centroid can fall *outside* a concave or multi-part polygon, which is common for coastal old towns. A polygon straddling a timezone or country boundary therefore resolves to whichever contains that point, **deterministically and identically on every run**; the resolved value is **stored**, so later reads never re-derive and never drift. It must never prompt.
  - **Pin the offline dataset**, which no ADR currently does: the IANA-timezone-from-point lookup needs a bundled timezone polygon set (a `timezonefinder`-style offline package — resolved-then-pinned per ADR-0007, and it must work with **no network**, since area resolution happens server-side but must stay reproducible in CI).
...
- [ ] T011 Implement `BundleManifestV1` in `commons/models.py` per the amended card — per-artifact hashes, `withheld[]`, embedded `TileSourceV1`, and the `integrity.manifest_sha256` canonicalization (sorted-key UTF-8 JSON with `integrity` omitted).
...
- [ ] T035 [US2] Implement `compiler/tiles.py` — `pmtiles extract` over the itinerary bbox + buffer, z0→maxzoom, build URL **resolved at run time** (never hotlinked), emitting an embedded `TileSourceV1` (ADR-0021).
...
- [ ] T038 [US2] Implement `compiler/quarantine.py` — drop every value where `licenses.bundleable(kind, license)` is false (**deriving** it for `Story`, which carries no stamp), refuse unstamped input, and record every removal into `withheld[]` with a reason (FR-011/FR-012/FR-021).
  - **`withheld[].reason` is a closed enum, never free text.** `withheld` ships inside an artifact the user downloads. License quarantine is safe to describe, but a free-text reason is a channel for anything a future withholding rule touches — including the private side of the PRD §13 #4 boundary — to leak out in a downloadable file. A closed enum (e.g. `license_forbids_redistribution`, `unstamped`, `source_unavailable`) says enough to render "needs connectivity" and cannot carry more than it should.
