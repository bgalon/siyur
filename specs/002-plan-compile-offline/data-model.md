...
1. **Feasibility holds before approval (FR-004/FR-005, SC-002)**: `Σ legs.distance_m ≤ budgets.walking_m` **and** total day span `≤ budgets.hours` **and** every stop's `[planned_start, planned_start+dwell_min]` falls inside its site's `opening_hours` window in area-local time. A violation is **named**, the plan is flagged, and approval is refused — never silently shipped. *(test: `test_feasibility.py`; DB `CHECK`; eval: feasibility)*
