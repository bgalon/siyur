...
**Undetermined — flagged, not decided**: (1) `Story` is **not** a `SourcedValue` — the card gives it a bare `source: SourceRef` with no `bundleable`, `confidence` or `observed_at`. Quarantine keys on `bundleable`, so either the filter derives it from `source` for stories (assumed above) or the card must add the stamp fields. `data-model.md` should say which; it is a card question, not a contract choice. (2) The card lists `stories` as **M1 / ≥1 story** while a place with no article correctly has none — the "≥1" is a fill-set aspiration, not a validation rule, and should be restated as such. (3) Nothing in spec or cards bounds a story's **length** or defines "adapted" against verbatim copying; `prompts/narration.md` v1 owns that, and the share-alike stamp holds either way.
