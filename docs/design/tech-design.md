...
- **`integrity.manifest_sha256` is canonicalized by [RFC 8785 (JCS)](https://www.rfc-editor.org/rfc/rfc8785)**, with the `integrity` key **omitted** before serialization — not nulled, not emptied. A named standard, because the obvious description ("UTF-8, sorted keys, no whitespace") is insufficient in the two ways that fire on **every** bundle: Python's `json.dumps` escapes the `©` in the ODbL attribution string by default (`ensure_ascii=True`) — every manifest — and renders `28.0` where `JSON.stringify` renders `28`, which bites *intermittently*, only when a `bbox` bound is integral. The intermittent one is the harder failure to diagnose. RFC 8785 pins escaping and number formatting too. Implementations: `rfc8785` (PyPI) / `canonicalize` (npm).
