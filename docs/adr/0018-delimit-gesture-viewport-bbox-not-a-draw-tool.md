...
- Bad / accepted cost: over-selection, and through `covered = count > 0` its effect on the reuse
  signal (above). Mitigated only by the user zooming appropriately, which is not a mitigation.
...
2. **The first delimitation an axis-aligned viewport rectangle demonstrably distorts** — in
   practice, a case where the over-selection above flips `coverage.covered` for a region the user
   did not research, or a walkable area whose useful shape is plainly not a rectangle.
