...
|---|---|---|---|
| **`opening-hours-py`** — Python bindings over the Rust crate `opening-hours` (`remi-dupre/opening-hours-rs`) | **MIT OR Apache-2.0** (both in the bundleable set) | **2.1.4, released 2026-07-07** — active | **PH yes** (embedded nager.Date holiday DB, country-scoped); **SH not documented as supported** |
...
- **Registry and schema-card debt (owned by another agent; recorded here so it is not lost):** `DATA-LICENSES.md` registers `opening_hours.js` / LGPL-3.0 as the feasibility evaluator, and `docs/data/poi-site.md` + `docs/data/itinerary.md` name it in prose. On acceptance those become **`opening-hours-py` / MIT OR Apache-2.0** and the LGPL row disappears. **DU-04 task.** Until it lands, the schema cards and this ADR disagree — and the cards are normally the field-level authority.
