...
def test_the_script_loads_the_model_key_from_the_keychain() -> None:
    """AGENTS.md promises this and, until FAIL-016, the script did not do it.

    Asserted on the *behaviour* — an already-set key survives — plus the presence of the
    Keychain lookup, because the load itself cannot run in CI (no Keychain, no item) and a
    test that silently skips there is the guarantee this entry exists to stop being absent.
    """
    # An explicitly-provided key must never be overwritten: CI and cloud secret injection
    # both set it, and this script must not fight them.
    assert _exported({"ANTHROPIC_API_KEY": "sentinel-not-a-real-key"}, "ANTHROPIC_API_KEY") == (
        "sentinel-not-a-real-key"
    )
    source = DEV_SH.read_text()
    assert "siyur-anthropic-api-key" in source, "the Keychain service name is gone"
    assert "security find-generic-password" in source, "the Keychain lookup is gone"


def test_the_script_never_prints_the_model_key() -> None:
    """A key that reaches a transcript is a leaked key (Constitution Article V).

    `dev.sh` prints a session cookie and a lot of guidance, so the risk is real: this pins
    that `ANTHROPIC_API_KEY` is only ever exported, never interpolated into an `echo`.
    """
    for line in DEV_SH.read_text().splitlines():
        stripped = line.strip()
        if stripped.startswith("#") or "ANTHROPIC_API_KEY" not in stripped:
            continue
        assert not stripped.startswith(("echo", "printf", "bold", "warn")), (
            f"dev.sh may print the model key: {stripped}"
        )
