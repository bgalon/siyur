...
| **Natural Earth 1:10m admin-0 countries** (`data/ne_10m_admin_0_countries.zip`, v5.1.1 — the country polygons `area.country_code` is derived from at resolve time, ADR-0029) | **public domain** (Natural Earth terms: *"All versions … are in the public domain. No permission is needed to use Natural Earth. Crediting the authors is unnecessary."*) | **No — none at all.** The least encumbered entry in this registry; the courtesy credit in `ATTRIBUTION.md` is a courtesy | **None.** No share-alike, no attribution, no notice. The committed 4.93 MB archive is redistributed with the repo and that is expressly permitted | ✅ yes (nothing retained in a bundle but the derived alpha-2 scalar; the file itself is a server-side input, not bundle content) | 2026-08-08 |
| **`timezonefinder` zone boundaries** (the polygons behind `area.timezone`; the wheel's data is built from **timezone-boundary-builder**, i.e. from OSM — library code MIT) | data **ODbL-1.0**; library **MIT** | No — see the share-alike note: nothing OSM-derived is *published* from this path | **Resolve-time only, and the only retained output is a scalar IANA id** — a fact, not a substantial extract, so no produced work and no derived database leaves the server. Nothing from this dataset enters a bundle. ⚠ if a future change ever emitted zone *geometry* into a bundle or an API response, that would be a produced work and this row changes. Related: ADR-0022 A6 pins `auto_timezone=False` inside `opening-hours-py` so the **evaluator** never reads its own copy of this data | ✅ yes (as a dependency; the boundary data itself is never bundled) | 2026-08-08 |
| **`holidays`** (python-holidays — public-holiday **coverage oracle and cross-check**, ADR-0029 Q2; `commons/frame.py` exposes the coverage set, T018 uses the dates) | **MIT** | Note in credits (retain the copyright + licence text) | **None — permissive, no copyleft.** Holiday dates are facts; the rule set is code and nothing propagates to `commons/` or to bundled data | ✅ yes (as a dependency) | 2026-08-08 |
| **Noto glyphs** (the bundled fonts under `glyphs/`, vendored per area by `compiler/tiles.py`; stamped `TileSourceV1.glyphs.license`) | **OFL-1.1** | No credit required — but **OFL §2 requires the copyright notice and licence text to accompany the fonts**, so `data/licenses/glyphs/OFL.txt` is emitted to **`glyphs/OFL.txt`** in every bundle and is inside the glyphs directory digest. Text captured verbatim from [`protomaps/basemaps-assets` `fonts/OFL.txt`](https://github.com/protomaps/basemaps-assets/blob/main/fonts/OFL.txt) — the licence shipped with the glyphs we vendor; its body matches the canonical SIL text at openfontlicense.org exactly (one historical trailing space aside) | **None on the data.** OFL's reciprocity binds *modified* fonts, which we do not produce; its one restriction is that the fonts may not be sold standalone. Nothing propagates to bundled data, the style or the tiles | ✅ yes | 2026-08-14 |
| **Protomaps sprite sheets** (the bundled map icons under `sprites/`; stamped `TileSourceV1.sprites.license`) | **MIT** | Note in credits, and **the copyright notice + licence text travel with the sheets**: `data/licenses/sprites/LICENSE.md` is emitted to **`sprites/LICENSE.md`** in every bundle, inside the sprites directory digest. Text captured verbatim from [`tangrams/icons` `LICENSE.md`](https://github.com/tangrams/icons/blob/master/LICENSE.md) (© 2017 Mapzen), which the basemaps-assets README names as the sheets' upstream | **None — permissive, no copyleft.** ⚠ These are **not** OFL: one shared registry row and one shared credit line put them under the fonts' license until 2026-08-14, which asserted the standalone-sale restriction over assets that never carried it. Two artifacts, two licenses, two credits | ✅ yes | 2026-08-14 |
