...
| **Noto glyphs** (the bundled fonts under `glyphs/`, vendored per area by `compiler/tiles.py`; stamped `TileSourceV1.glyphs.license`) | **OFL-1.1** | No — a courtesy credit; `OFL.txt` ships in the bundle beside the glyphs it covers | **None on the data.** OFL's reciprocity binds *modified* fonts, which we do not produce; its one restriction is that the fonts may not be sold standalone. Nothing propagates to bundled data, the style or the tiles | ✅ yes | 2026-08-14 |
| **Protomaps sprite sheets** (the bundled map icons under `sprites/`; stamped `TileSourceV1.sprites.license`) | **MIT** | Note in credits (retain the copyright + licence text) | **None — permissive, no copyleft.** ⚠ These are **not** OFL: one shared registry row and one shared credit line put them under the fonts' license until 2026-08-14, which asserted the standalone-sale restriction over assets that never carried it. Two artifacts, two licenses, two credits | ✅ yes | 2026-08-14 |
